...
import {
  useFetchAllApiControllerListQuery,
...
import {
  setControllerAccessApiKeyId,
  setControllerAccessBusinessId,
  setControllerAccessCategoryId,
  setControllerAccessJwtUserId,
} from "../../store/reducer/ensureAccess/slice";
import { AppSearchableDropdown } from "../AppSearchableDropdown";

export const ControllerAccessFilter: FC = () => {
  ...
  const {
    data: existingApiControllerList,
    isLoading: isApiControllerLoading,
    isFetching: isApiControllerFetching,
  } = useFetchAllApiControllerListQuery();
  const {
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div>
        <AppSearchableDropdown
          ...
          placeHolder="API controller name"
          freeSolo={false}
          optionList={
            !(isApiControllerLoading || isApiControllerFetching)
              ? existingApiControllerList?.map((ac) => ac.name) || []
              : []
          }
          onInputChange={(value) =>
            dispatch(
              setControllerAccessCategoryId(
                existingApiControllerList?.find((ac) => ac.name === value)
                  ?.id || 0
              )
            )
          }
